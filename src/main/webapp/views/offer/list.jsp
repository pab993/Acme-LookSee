...
<!-- Listing table -->
	...
	<display:table name = "offers" id = "row" requestURI = "${requestURI}" pagesize = "5" class = "displaytag" >
			...
			<display:column>
				<jstl:if test="${actual lt row.deadline}">
					<a href="application/create.do?offerId=${row.id}">
						<spring:message code="offer.application" />
					</a>
				</jstl:if>
			</display:column>	
			</security:authorize>
			...
			<display:column>
					<a href="application/listByOffer.do?offerId=${row.id}">
